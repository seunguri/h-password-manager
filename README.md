...
<img width="1356" height="693" alt="image" src="https://github.com/user-attachments/assets/cbc06e5b-a40a-44ad-8504-ec7dc479e850" />
...
- **계정 관리**: 서비스명, 아이디, 비밀번호, 비밀번호 변경일·만료, 메모, 태그 — 추가·조회·수정·삭제 및 목록 순서 변경
...
- **검색·필터**: 서비스명, 아이디, 태그, 메모로 검색하고 태그로 좁히기
- **단축키**
  - `Ctrl+Shift+P`: 앱을 찾지 않고 창 열기
  - `Ctrl+F`: 검색
  - `Ctrl+Shift+C`: 맨 앞(좌측 상단) 계정의 비밀번호 바로 복사
  - `Ctrl+N`: 새 계정 추가
  - `F5`: 대시보드 새로고침
  - `Enter`: 저장
  - `Esc`: 창 최소화
...
**실행 파일 공유:** 네이티브 DLL과 리소스를 모두 포함하는 **단일 `PasswordProtector.exe`** 하나만 생성되므로, 그 exe 파일만 복사해서 공유하면 됩니다. (`PasswordProtector.pdb`는 디버그용이라 없어도 됩니다.)
...
3. 검색·태그로 항목 찾기
4. 카드를 클릭하면 비밀번호가 복사되고, `✏️ 수정` 버튼으로 상세 보기·수정·삭제
5. 창 닫기 시 트레이로 최소화
